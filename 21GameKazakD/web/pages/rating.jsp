...
<table>
    <tr>
        <th><fmt:message key="rating.table.place"/></th>
        <th><fmt:message key="rating.table.user"/></th>
        <th><fmt:message key="rating.table.rating"/></th>
    </tr>
    ...
    <c:forEach items="${requestScope.users}" var="user">
        <tr>
            ...
            <td><c:out value="${user.rate}"></c:out></td>
    ...
    <div id="pagination" class="w3-bar w3-border w3-green w3-margin-top">
        <c:choose>
            <c:when test="${currentPage-1 < 1}">
                <a href="/controller?command=showRating&page=${currentPage}" class="w3-button">&laquo;</a>
            </c:when>
            <c:otherwise>
                <a href="/controller?command=showRating&page=${currentPage-1}" class="w3-button">&laquo;</a>
            </c:otherwise>
        </c:choose>
        ...
        <c:forEach var="page" begin="1" end="${pagesCount}">
            <c:choose>
                <c:when test="${currentPage==page}">
                    <a href="/controller?command=showRating&page=${page}" class="w3-button w3-cyan">${page}</a>
                </c:when>
                <c:otherwise>
                    <a href="/controller?command=showRating&page=${page}" class="w3-button">${page}</a>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        <c:choose>
            <c:when test="${currentPage+1 > pagesCount}">
                <a href="/controller?command=showRating&page=${currentPage}" class="w3-button">&raquo;</a>
            </c:when>
            <c:otherwise>
                <a href="/controller?command=showRating&page=${currentPage+1}" class="w3-button">&raquo;</a>
            </c:otherwise>
        </c:choose>
    </div>
</div>
